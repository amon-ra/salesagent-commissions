...
class salesagent_common(orm.Model):
    ...
    def recognized_commission(self, cr, uid, customer_id, salesagent_id, product_id):
        ...
        if lst_commission_product_customer:
            ...
            return commission
        # If customer has a special commission for the product category
        product_category_data = self.pool.get('product.product').browse(cr, uid, product_id).categ_id
        if product_category_data:
            lst_commission_product_category_customer = partner_product_category_commission_obj.search(cr, uid,
                [('name', '=', product_category_data.id), ('partner_id', '=', customer_id)])
            if lst_commission_product_category_customer:
                commission = partner_product_category_commission_obj.browse(cr, uid, lst_commission_product_category_customer[0]).commission
                return commission
        ...
        if lst_commission_product_customer:
            commission = partner_product_commission_obj.browse(cr, uid, lst_commission_product_customer[0]).commission
            return commission
        # If salesagent has a special commission for the product category
        product_category_data = self.pool.get('product.product').browse(cr, uid, product_id).categ_id
        if product_category_data:
            lst_commission_product_category_customer = partner_product_category_commission_obj.search(cr, uid,
                [('name', '=', product_category_data.id), ('partner_id', '=', salesagent_id)])
            if lst_commission_product_category_customer:
                commission = partner_product_category_commission_obj.browse(cr, uid, lst_commission_product_category_customer[0]).commission
                return commission
        ...
        if commission:
            return commission
        # If product category has a special generic commission
        ...
        if product_data.categ_id:
            ...
            commission = self.pool.get('product.category').browse(cr, uid, categ_id).commission
            if commission:
                return commission
        return 0.0
